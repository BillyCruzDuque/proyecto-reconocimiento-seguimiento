...
url = 'http://vitalia.hopto.org:3000/api/registros'
...
def horario(nombre):
    with open('Horario.csv', 'r+') as h:
        ...
        if nombre not in listanombres:
            ...
            params = {'id_empleado': nombre, 'fecha': fecha, 'hora_entrada': '20:13:12'}

            r1 = requests.post(url, json=params, headers=headers)

            registro = (nombre, fecha, hora)
            cursor1.execute(sql, registro)
